...
> **Engine note.** The decoding/streaming/audio engine is now **`nova2026.auditory`**
> (NOVA2026 `audio_v2`), driven through `scripts.auditory` (`convert` / `train` /
> `replay` / `live`). The standalone script names below (`run_viability.py`,
> `demo_realtime.py`, `calibrate.py`, `pretrain_generic.py`, `live_demo.py`) are
> **superseded** — the day-of flow is: `scripts.auditory.convert` → `train` → `live`
> (on the cap via `StreamSession`) or `replay` (no hardware), then `demo/build_demo.py`
> for the browser view. See `INTEGRATION.md`. The strategy and honest caveats below
> still hold.
...
- Per-person calibration only helps with **enough** data and a rig that matches the
  one it was recorded on. In our cross-person test it did **not** surpass the generic
  decoder within the lengths we measured (short calibrations were worse; even several
  minutes topped out in the high-60s%). Treat "personalize" as an experiment to try
  when you have time and a matched rig — not a reliable way to beat generic on the day
  (audit ISS-09). The headline 82% is a within-subject leave-one-trial-out number, a
  different measurement from day-of cross-person calibration.

**Recommended plan:** during practice, record a handful of teammates on the ANT Neuro
cap and build a generic decoder once:

```bash
python pretrain_generic.py --data-dir /path/to/recordings --subjects P1 P2 P3 \
       --out-file generic_decoder.npz
```

Then on the day a judge just puts on the cap — no calibration:

```bash
python live_demo.py --eeg lsl --audio mic --decoder results/generic_decoder.npz
```
...
```bash
python calibrate.py --data-dir /path/to/recordings --eeg lsl --minutes 3 \
       --files talkerA.wav talkerB.wav --out-file personal_decoder.npz
python live_demo.py --eeg lsl --audio mic --decoder results/personal_decoder.npz
```

Add `--base results/generic_decoder.npz` to `calibrate.py` to warm-start from the
