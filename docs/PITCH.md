...
> **Engine note.** The pipeline described here is now implemented in
> **`nova2026.auditory`** (NOVA2026 `audio_v2`). Any command below that names a
> standalone script (`run_viability.py`, `verify.py`, `demo_realtime.py`,
> `calibrate.py`, `pretrain_generic.py`, `live_demo.py`) is **superseded** — use
> `scripts.auditory` (`convert` / `train` / `replay` / `evaluate` / `live`) and this
> repo's `demo/build_demo.py`. See `INTEGRATION.md`. The concept, viability story and
> trade-offs below remain valid; the numbers in `results/` are from the earlier offline
> feature contract and should be re-run through `scripts.auditory` for current figures.
...
```bash
# today, fully offline:
python demo_realtime.py --data-dir /path/to/AAD --subject S3 --trial 4 --output wav
# build day, live:
...
- **Personalized — optional, needs enough data and a matched rig.** Guide the listener
  to attend each talker, record, and train their own decoder (`calibrate.py`). In our
  cross-person measurement this did **not** beat the generic decoder within the lengths
  tested: 1 min ≈ 57% (worse than generic), and even several minutes topped out in the
  high-60s%, still below generic's ~72%.

We measured the trade-off directly (see `results/calibration_analysis.png`, cross-person):
short from-scratch calibration is **worse** than the generic decoder, and none of the
tested lengths overtook it. So the rule is: **generic decoder for walk-up judges**;
treat personalization as an experiment for when you have ample data and a rig that
matches the recordings, not a reliable way to beat generic on the day. The **82%**
headline is a *separate* quantity — a within-subject leave-one-trial-out viability
result — and must not be read as a cross-person calibration outcome (audit ISS-09).
...
python run_viability.py --data-dir /path/to/AAD   # reproduce the viability result
python verify.py        --data-dir /path/to/AAD   # the sanity controls
python demo_realtime.py --data-dir /path/to/AAD --subject S3 --trial 4  # the demo
```
