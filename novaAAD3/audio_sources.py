...
def _to_mono_float(x):
    """Convert a WAV array to mono float in [-1, 1].

    FIX (review finding 5): normalize according to the ORIGINAL PCM dtype *before*
    down-mixing. The previous version averaged channels first, which turned integer
    stereo into float and silently skipped normalization (a value of 16384 stayed
    16384 instead of ~0.5). Unsigned PCM (e.g. uint8) is also correctly centered.
    """
    x = np.asarray(x)
    if np.issubdtype(x.dtype, np.integer):
        info = np.iinfo(x.dtype)
        if info.min == 0:                       # unsigned PCM: centre then scale
            half = (info.max + 1) / 2.0
            x = (x.astype(np.float64) - half) / half
        else:                                   # signed PCM: /2^(bits-1)
            x = x.astype(np.float64) / -float(info.min)
    else:
        x = x.astype(np.float64)                # already float PCM
    if x.ndim > 1:                              # down-mix AFTER normalization
        x = x.mean(axis=1)
