...
def _finish(args, out, log):
    ...
    if log:
        # NOTE: demo_realtime does not randomize A/B ordering, so this is only a rough
        # demonstration indicator, NOT a validated accuracy. Use evaluate.py / run_viability
        # for real, randomized, null-controlled numbers.
        favored = sum(1 for _, cA, cB, att, *_ in log if att == 0)
        print(f"[demo indicator only] {favored}/{len(log)} decisions favoured source A.")
