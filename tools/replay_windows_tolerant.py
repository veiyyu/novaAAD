"""Same as scripts.auditory.runner.replay_windows, but forwards the channel-
quality tolerance knobs (check_channels/max_bad_channels/exclude_channels)
that stream_config/AuditoryProcessor already support and scripts/auditory/
live.py already exposes on the CLI, but the offline replay_windows() does
not. No vendored code is changed -- this literally re-runs the same
stream_config -> AuditoryProcessor -> EnvelopeExtractor/EnvelopeBuffer chain
runner.replay_windows uses, just with those three extra keyword arguments
threaded through, because real (not synthetic) EEG has real artifacts that
the new stricter per-channel bad-electrode policy (today's re-vendor) now
correctly flags, and the offline path has nowhere to say "tolerate up to N
faulting channels" the way the live path can.
"""
import numpy as np
...
def replay_windows_tolerant(
    ...
    processing = stream_config(
        trial, config, history, step,
