# novaAAD 3 — review fixes + NOVA2026 integration
...
Run: `PYTHONPATH=. python nova/test_adapter.py` → **11/11 pass**, and
`PYTHONPATH=. python nova/test_equivalence.py` → **3/3 pass** (numpy/scipy only;
...
- **`nova/aad_adapter.py`**
  ...
  - `ReliabilityController` — turns `AADResult`s into per-talker audio gains, with
    two stability guards (review finding 6): **minimum dwell** — a new talker
    becomes the committed pick only after it wins `min_switch_windows` consecutive
    confident decisions, so a single spurious window (or lone abstain) never flips
    the audio — and **freshness at the moment of action** — a decision past its
    `expires_at` decays to a neutral (both-audible) mix even if no new result
    arrives (closes the review's "stale result still acts" gap). Gains glide to
    their targets over a short ramp; they never jump.
...
PYTHONPATH=. python nova/test_adapter.py         # 11/11 — NOVA tube + dwell smoothing
PYTHONPATH=. python nova/test_equivalence.py     # 3/3   — offline == streamed (Tests 1-2)
python evaluate.py --data-dir /path/to/AAD       # honest accuracy + asserted null controls
```

**Offline == streamed (review Tests 1-2).** `nova/test_equivalence.py` proves a
window assembled from arbitrary streamed chunks is byte-identical to the offline
slice of the same samples and yields identical decoder output — so the live path
reproduces the offline result. Run `evaluate.py` (offline accuracy on real data)
alongside it to cover both halves.
