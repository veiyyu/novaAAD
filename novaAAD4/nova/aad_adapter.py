...
class ReliabilityController:
    ...
    evidence, with temporal smoothing so momentary noise never flips the audio.

    Provides ``gains(now_source_time)`` for an independent audio path. The audio
    callback calls ``gains()`` every block; this class only updates *targets*.

    Two guards against unstable steering (review findings 6):
    - **Minimum dwell** — a *new* talker becomes the committed pick only after it
      wins ``min_switch_windows`` consecutive valid, confident decisions. A single
      spurious window (or a lone abstain) never causes a switch. With
      ``min_switch_windows=1`` (default) behaviour is unchanged: commit on the
      first confident pick.
    - **Freshness at action time** — a committed decision past its ``expires_at``
      (relative to the current source time) decays the mix back to neutral even if
      a new result never arrives, closing the "stale result still acts" gap.
    ...
    def __init__(self, n_sources: int = 2, atten_db: float = ATTEN_DB,
        ...
        self.min_switch = max(1, int(min_switch_windows))
        self._committed: int | None = None    # talker currently amplified
        self._cand: int | None = None         # talker accumulating consecutive wins
        self._cand_count: int = 0
        self._expires_at: float = -np.inf

    def update(self, result: AADResult) -> None:
        """NOVA on-result sink: accept a fresh, valid, confident pick — but only
        commit a *change* after the minimum dwell of consecutive agreeing windows.
        Invalid / uncertain results break a candidate run and leave the committed
        pick to age out via freshness."""
        if not (getattr(result, "valid", False) and result.pick is not None):
            self._cand, self._cand_count = None, 0         # abstain breaks the run
            return
        p = int(result.pick)
        self._expires_at = float(result.expires_at)         # committed pick stays fresh
        if p == self._committed:
            self._cand, self._cand_count = None, 0          # already steering there
            return
        self._cand_count = self._cand_count + 1 if p == self._cand else 1
        self._cand = p
        if self._cand_count >= self.min_switch:             # dwell satisfied -> switch
            self._committed = p
    ...
    def _refresh_target(self, now_source_time: float) -> None:
        ...
        if self._committed is None or stale:               # freshness re-check at action
            self._target = np.full(self.n, self.neutral)
        else:
            self._target = np.full(self.n, self.duck)
            self._target[self._committed] = 1.0
